...
public final class JPCKeyboardAdapter extends KeyAdapter {
    ...
    public void reset(boolean numLock, boolean capsLock, boolean scrollLock) {
        ...
        // Set locking key states (skip in headless mode)
        try {
            ...
            // Ignore - running headless without display
        }
    ...
    // <editor-fold defaultstate="collapsed" desc="Programmatic keyboard control for headless operation">

    /**
     * Press a key by keyCode (for headless/programmatic control)
     * @param keyCode AWT KeyEvent key code (e.g., KeyEvent.VK_A)
     * @param location Key location (e.g., KeyEvent.KEY_LOCATION_STANDARD)
     * @return true if key was found and pressed
     */
    public synchronized boolean pressKeyCode(int keyCode, int location) {
        JPCVirtualKey key = m_bindings.get(getMappingValue(keyCode, location));
        if (key != null) {
    ...
    /**
     * Press a key by keyCode with standard location
     */
    public synchronized boolean pressKeyCode(int keyCode) {
        // Try standard location first, then left, then numpad
        if (pressKeyCode(keyCode, KeyEvent.KEY_LOCATION_STANDARD)) return true;
        if (pressKeyCode(keyCode, KeyEvent.KEY_LOCATION_LEFT)) return true;
        if (pressKeyCode(keyCode, KeyEvent.KEY_LOCATION_NUMPAD)) return true;
        return false;
    }

    /**
     * Release a key by keyCode (for headless/programmatic control)
     * @param keyCode AWT KeyEvent key code
     * @param location Key location
     * @return true if key was found and released
     */
    public synchronized boolean releaseKeyCode(int keyCode, int location) {
        JPCVirtualKey key = m_bindings.get(getMappingValue(keyCode, location));
        if (key != null) {
            if (m_pressedKeys.contains(key)) {
                releaseKey(key);
    ...
    /**
     * Release a key by keyCode with standard location
     */
    public synchronized boolean releaseKeyCode(int keyCode) {
        // Try standard location first, then left, then numpad
        if (releaseKeyCode(keyCode, KeyEvent.KEY_LOCATION_STANDARD)) return true;
        if (releaseKeyCode(keyCode, KeyEvent.KEY_LOCATION_LEFT)) return true;
        if (releaseKeyCode(keyCode, KeyEvent.KEY_LOCATION_NUMPAD)) return true;
        return false;
    }

    /**
     * Tap a key (press and release) by keyCode
     */
    public synchronized void tapKeyCode(int keyCode) {
        pressKeyCode(keyCode);
        releaseKeyCode(keyCode);
    }

    /**
     * Tap a key with specific location
     */
    public synchronized void tapKeyCode(int keyCode, int location) {
        pressKeyCode(keyCode, location);
        releaseKeyCode(keyCode, location);
    }

    /**
     * Type a character by converting to keyCode
     */
